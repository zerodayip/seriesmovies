...
def process_files(paths, cache_file, is_series=True):
    ...
    for path in paths:
        ...
        for key, val in entries.items():
            ...
            # Eksik veri varsa IMDb'den al
            if not cache[key].get("imdb_id") or not cache[key].get("poster"):
                imdb_id, poster = search_imdb_exact(key, is_series)
                if imdb_id and not cache[key].get("imdb_id"):
                    cache[key]["imdb_id"] = imdb_id
                    updated = True
                if poster and not cache[key].get("poster"):
